...
def extract_secondary_identifier(properties):
    return '%s: %s' % (properties.get('search-engine', None), properties.get('search-type', None))
...
def compile_report(generator, sources, data_start=None, data_end=None, date_type='created'): # pylint: disable=too-many-locals, too-many-branches, too-many-statements
    now = arrow.get()
    filename = tempfile.gettempdir() + os.path.sep + 'search_historian_export_' + str(now.timestamp) + str(old_div(now.microsecond, 1e6)) + '.zip'
        ...
        for source in sources:
            ...
            with io.open(secondary_filename, 'w', encoding='utf-8') as outfile:
                ...
                for point in points:
                    ...
                    time_zone = properties.get('passive-data-metadata', {}).get('timezone', settings.TIME_ZONE)

                    created = point.created.astimezone(pytz.timezone(time_zone))
                    recorded = point.recorded.astimezone(pytz.timezone(time_zone))
                    ...
                    row.append(properties.get('search-engine', ''))
                    row.append(properties.get('search-type', ''))
                    row.append(properties.get('query', ''))
                    row.append(properties.get('url', ''))

                    user_agent = properties.get('passive-data-metadata', {}).get('generator', None)
